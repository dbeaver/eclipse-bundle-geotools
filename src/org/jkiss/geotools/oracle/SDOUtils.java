 ...
 * please see SDOGeometryConverter for an example of this.
 *
...
public final class SDOUtils {
     ...
     *       none/last
     *   <li><b><code>tt:</code></b> See the SDOGeometryType. constants defined in this class
     * </ul>
     *
    ...
    /**
     * Return SDOGeometryType as defined by SDO_GTYPE (represents geometry type).
     *
     * <p>SDOGeometryType is used to represent the type of the JTS Geometry:
     *
     * <pre><code>
     ...
     * </code></pre>
     *
     * @return <code>SDOGeometryType</code> representing <code>geom</code>
     * @throws IllegalArgumentException If SDO_GTYPE can not be represetned by JTS
     */
    public static int TT(Geometry geom) {
        if (geom == null) {
            return SDOGeometryType.UNKNOWN; // UNKNOWN
        } else if (geom instanceof Point) {
            return SDOGeometryType.POINT;
        } else if (geom instanceof LineString) {
            return SDOGeometryType.LINE;
        } else if (geom instanceof Polygon) {
            return SDOGeometryType.POLYGON;
        } else if (geom instanceof MultiPoint) {
            return SDOGeometryType.MULTIPOINT;
        } else if (geom instanceof MultiLineString) {
            return SDOGeometryType.MULTILINE;
        } else if (geom instanceof MultiPolygon) {
            return SDOGeometryType.MULTIPOLYGON;
        } else if (geom instanceof GeometryCollection) {
            return SDOGeometryType.COLLECTION;
        }
     ...
     * objects.
     *
     * <p>You cannot use this with LRS SDOCoordinates
     *
    ...
    private static void addElemInfo(List elemInfoList, Point point, final int STARTING_OFFSET) {
        ...
        addInt(elemInfoList, SDOGeometryEType.POINT);
    ...
    private static void addElemInfo(List elemInfoList, LineString line, final int STARTING_OFFSET) {
        addInt(elemInfoList, STARTING_OFFSET);
        addInt(elemInfoList, SDOGeometryEType.LINE);
    ...
    private static void addElemInfo(
        ...
        if (HOLES == 0) {
            ...
            addInt(elemInfoList, elemInfoInterpretation(polygon, SDOGeometryEType.POLYGON_EXTERIOR));
        ...
        for (int i = 1; i <= HOLES; i++) {
            ...
            addInt(elemInfoList, offset);
            addInt(elemInfoList, SDOGeometryEType.POLYGON_INTERIOR);
            addInt(elemInfoList, elemInfoInterpretation(ring, SDOGeometryEType.POLYGON_INTERIOR));
            offset += (ring.getNumPoints() * LEN);
        }
    }
    ...
    private static void addElemInfo(
        ...
        addInt(elemInfoList, STARTING_OFFSET);
        addInt(elemInfoList, SDOGeometryEType.POINT);
        addInt(elemInfoList, elemInfoInterpretation(points, SDOGeometryEType.POINT));
    }
     ...
     *   <li>We are not allowed to mix 1 digit and 4 digit values for SDOGeometryEType and GTYPE in a single
     *       geometry
     * </ul>
     *
    ...
    protected static int elemInfoEType(Geometry geom) {
        switch (TT(geom)) {
            case SDOGeometryType.UNKNOWN:
                return SDOGeometryEType.CUSTOM;

            case SDOGeometryType.POINT:
                return SDOGeometryEType.POINT;

            case SDOGeometryType.LINE:
                return SDOGeometryEType.LINE;

            case SDOGeometryType.POLYGON:
                return isExterior((Polygon) geom)
                        ? SDOGeometryEType.POLYGON_EXTERIOR // cc order
                        : SDOGeometryEType.POLYGON_INTERIOR; // ccw order
     ...
     * 4         N > 1    Compound Line String made of N (SDOGeometryEType=2) lines and arcs
     * 1005/2005 N > 1    Polygon defined by (SDOGeometryEType=2) lines and arcs
     *
     *
     * </code></pre>
     *
     ...
     *
     * <p>Provides the INTERPRETATION value for the ELEM_INFO triplet of (STARTING_OFFSET, SDOGeometryEType,
     * INTERPRETATION).
     *
     * @param geom Geometry to encode
     * @param etype SDOGeometryEType value requiring an INTERPREATION
    ...
    public static int elemInfoInterpretation(Geometry geom, final int etype) {
        switch (etype) {
            case SDOGeometryEType.CUSTOM: // customize for your own Geometries
                break;

            case SDOGeometryEType.POINT:
            ...
            case SDOGeometryEType.POLYGON:
            case SDOGeometryEType.POLYGON_EXTERIOR:
            case SDOGeometryEType.POLYGON_INTERIOR:
            ...
            case SDOGeometryEType.COMPOUND:
                throw new IllegalArgumentException(
                        "JTS LineStrings are not composed of curves and lines.");

            case SDOGeometryEType.COMPOUND_POLYGON:
            case SDOGeometryEType.COMPOUND_POLYGON_INTERIOR:
            case SDOGeometryEType.COMPOUND_POLYGON_EXTERIOR:
                throw new IllegalArgumentException(
    ...
    */
    ...
    /**
     * Ensure Ring of SDOCoordinates are in a counter Orientationwise order.
     *
     ...
     * @param ring Ring of SDOCoordinates
    ...
    /**
     * Ensure Ring of SDOCoordinates are in a Orientationwise order.
     *
     * <p>If the Coordinate need to be reversed a copy will be returned.
     *
     ...
     * @param coords SDOCoordinates
     * @param GTYPE Encoding of <b>D</b>imension, <b>L</b>RS and <b>SDOGeometryType</b>ype
     * @param elemInfo
     * @param triplet
     */
    ...
    /** Access SDOGeometryType (for geometry type) as encoded in GTYPE */
    ...
    /**
     * SDOGeometryEType access for the elemInfo triplet indicated.
     *
     * <p>
     *
     * @see SDOGeometryEType for an indication of possible values
     * @param elemInfo
     * @param triplet
     * @return SDOGeometryEType for indicated triplet
     */
    ...
    /** SDOCoordinates from <code>(x,y,x2,y2,...)</code> ordinates. */
    public static Coordinate[] asCoordinates(double[] ordinates) {
        return asCoordinates(ordinates, 2);
    }

    /** SDOCoordinates from a <code>(x,y,i3..,id,x2,y2...)</code> ordinates. */
    ...
    public static CoordinateSequence coordinates(
        ...
        final int D = SDOUtils.D(GTYPE);
        final int L = SDOUtils.L(GTYPE);
        final int TT = SDOUtils.TT(GTYPE);
...
*/
     ...
     *
     * <p>To produce two dimension SDOCoordinates pass in <code>null</code> for z
     */
    public static CoordinateSequence coordiantes(
        CoordinateSequenceFactory f, SDOOrdinateList x, SDOOrdinateList y, SDOOrdinateList z) {
     ...
     */
    public static CoordinateSequence coordiantes(
        CoordinateSequenceFactory f, SDOAttributeList x, SDOAttributeList y, SDOAttributeList z) {
        final int LENGTH = x.size();
        Coordinate[] array = new Coordinate[LENGTH];
     ...
     *
     * <p>To produce three dimension SDOCoordinates pass in <code>null</code> for z
     *
     ...
     *     SDOUtils#coordiantes(CoordinateSequenceFactory, SDOOrdinateList, SDOOrdinateList, SDOOrdinateList,
     *     SDOOrdinateList) coordiantes() with just a SDOOrdinateList of measures}! Construct
    ...
    /**
     * Decode geometry from provided SDOUtils encoded information.
     *
     * <p>
     *
    ...
    public static Geometry create(
        ...
        if ((L == 0) && (TT == 01) && (point != null) && (elemInfo == null)) {
            ...
            elemInfo = new int[] {1, SDOGeometryEType.POINT, 1};
        } else {
            int element = 0;
    ...
    /**
     * Consturct geometry with SDOUtils encoded information over a CoordinateList.
     *
    ...
    private static Point createPoint(
        ...
        if (etype != SDOGeometryEType.POINT)
            throw new IllegalArgumentException(
                    "SDOGeometryEType " + etype + " inconsistent with expected POINT");
        if (INTERPRETATION != 1) {
     ...
     */
    private static Geometry createLine(
            GeometryFactory gf,
            final int GTYPE,
            final int SRID,
            final int[] elemInfo,
        ...
        if (etype != SDOGeometryEType.LINE && etype != SDOGeometryEType.COMPOUND) return null;

        Geometry result;
        if (etype == SDOGeometryEType.LINE && INTERPRETATION == 1) {
        ...
        } else if (etype == SDOGeometryEType.LINE && INTERPRETATION == 2) {
            CoordinateSequence subList =
                    subList(
                            gf.getCoordinateSequenceFactory(),
                            coords,
     ...
     *         <li>SDOGeometryEType: 1003 (exterior) or 3 (polygon w/ counter Orientationwise ordinates)
     *         <li>INTERPRETATION: 1 for strait edges, 3 for rectanlge
     *       </ul>
     *   <li>Interior Polygon Ring(s): remaining triplets:
     *       <ul>
     *         <li>STARTING_OFFSET: position in ordinal ordinate array
     *         <li>SDOGeometryEType: 2003 (interior) or 3 (polygon w/ OrientationWise ordinates)
     *         <li>INTERPRETATION: 1 for strait edges, 3 for rectanlge
     *       </ul>
     * </ul>
     *
     ...
     * @param coords SDOCoordinates to interpret using elemInfo
     * @return Polygon as encoded by elemInfo, or null when faced with and encoding that can not be
     *     captured by JTS
     * @throws IllegalArgumentException When faced with an invalid SDOUtils encoding
     */
    private static Polygon createPolygon(
        ...
        if (!(1 <= STARTING_OFFSET && STARTING_OFFSET <= ordinateSize(coords, GTYPE))) {
            throw new IllegalArgumentException(
                ...
                "SDOGeometryEType {0} must be expected POLYGON or POLYGON_EXTERIOR (one of {1})",
                eTYPE,
                new int[] {
                    SDOGeometryEType.COMPOUND_POLYGON_EXTERIOR,
                    SDOGeometryEType.COMPOUND_POLYGON,
                    SDOGeometryEType.POLYGON,
                    SDOGeometryEType.POLYGON_EXTERIOR,
                    SDOGeometryEType.FACE_EXTERIOR
                });
        if ((eTYPE != SDOGeometryEType.COMPOUND_POLYGON_EXTERIOR)
                && (eTYPE != SDOGeometryEType.COMPOUND_POLYGON)
        ...
        for (int i = triplet + 1; (etype = ETYPE(elemInfo, i)) != -1; ) {
            if (etype == SDOGeometryEType.POLYGON_INTERIOR) {
                rings.add(createLinearRing(gf, GTYPE, SRID, elemInfo, i, coords));
                i++;
            } else if (etype == SDOGeometryEType.COMPOUND_POLYGON_INTERIOR) {
                int subelements = INTERPRETATION(elemInfo, i);
                rings.add(createLinearRing(gf, GTYPE, SRID, elemInfo, i, coords));
                i = i + subelements + 1;
            } else if (etype
                    == SDOGeometryEType.POLYGON) { // nead to test Orientationwiseness of Ring to see if it
                // is
     ...
     * <ul>
     *   <li>STARTING_OFFSET: position in ordinal ordinate array
     *   <li>SDOGeometryEType: 1003 (exterior) or 2003 (interior) or 3 (unknown order)
     *   <li>INTERPRETATION: 1 for strait edges, 3 for rectanlge
     * </ul>
     *
     * <p>The dimensionality of GTYPE will be used to transalte the <code>STARTING_OFFSET</code>
    ...
    private static LinearRing createLinearRing(
        ...
        if (!(STARTING_OFFSET >= 1) || !(STARTING_OFFSET <= LENGTH))
            throw new IllegalArgumentException(
                ...
                new int[] {
                    SDOGeometryEType.COMPOUND_POLYGON,
                    SDOGeometryEType.COMPOUND_POLYGON_EXTERIOR,
                    SDOGeometryEType.COMPOUND_POLYGON_INTERIOR,
                    SDOGeometryEType.POLYGON,
                    SDOGeometryEType.POLYGON_EXTERIOR,
                    SDOGeometryEType.POLYGON_INTERIOR,
                    SDOGeometryEType.FACE_EXTERIOR
                });
        if ((eTYPE != SDOGeometryEType.COMPOUND_POLYGON_EXTERIOR)
                && (eTYPE != SDOGeometryEType.COMPOUND_POLYGON)
                && (eTYPE != SDOGeometryEType.COMPOUND_POLYGON_INTERIOR)
                && ((INTERPRETATION < 1) || (INTERPRETATION > 4))) {
        ...
        if (eTYPE == SDOGeometryEType.COMPOUND_POLYGON_EXTERIOR || eTYPE == SDOGeometryEType.COMPOUND_POLYGON_INTERIOR) {
            int triplets = INTERPRETATION;
            List<LineString> components = new ArrayList<>(triplets);
/*
            for (int i = 1; i <= triplets; i++) {
                LineString component =
                    (LineString) createLine(gf, GTYPE, SRID, elemInfo, triplet + i, coords, i < triplets);
                components.add(component);
            }
*/
            ...
            ring = (LinearRing) gf.createLinearRing(coordSeq);
...
*/
     ...
     * <ul>
     *   <li>STARTING_OFFSET: position in ordinal ordinate array
     *   <li>SDOGeometryEType: 1 for Point
     ...
     * <ul>
     *   <li>STARTING_OFFSET: position in ordinal ordinate array
     *   <li>SDOGeometryEType: 2 for Line
        ...
        if (!(STARTING_OFFSET >= 1) || !(STARTING_OFFSET <= LENGTH))
            throw new IllegalArgumentException(
                    "ELEM_INFO STARTING_OFFSET "
                            ...
                            + " inconsistent with ORDINATES length "
                            + coords.size());
        if (!(eTYPE == SDOGeometryEType.LINE))
            throw new IllegalArgumentException(
                    "SDOGeometryEType " + eTYPE + " inconsistent with expected LINE");
        if (!(INTERPRETATION == 1)) {
     ...
     * <ul>
     *   <li>STARTING_OFFSET: position in ordinal ordinate array
     *   <li>SDOGeometryEType: 2003 or 3 for Polygon
    ...
    private static MultiPolygon createMultiPolygon(
        ...
        if (!(STARTING_OFFSET >= 1) || !(STARTING_OFFSET <= LENGTH))
            throw new IllegalArgumentException(
                    "ELEM_INFO STARTING_OFFSET "
                            ...
                            + " inconsistent with ORDINATES length "
                            + coords.size());
        if (!(eTYPE == SDOGeometryEType.POLYGON)
                && !(eTYPE == SDOGeometryEType.POLYGON_EXTERIOR)
                && !(eTYPE == SDOGeometryEType.FACE_EXTERIOR)
                && !(eTYPE == SDOGeometryEType.FACE_INTERIOR)
                && !(eTYPE == SDOGeometryEType.COMPOUND_POLYGON)
                && !(eTYPE == SDOGeometryEType.COMPOUND_POLYGON_EXTERIOR))
            throw new IllegalArgumentException(
                    "SDOGeometryEType " + eTYPE + " inconsistent with expected POLYGON or POLYGON_EXTERIOR");
        if (!(eTYPE == SDOGeometryEType.COMPOUND_POLYGON)
                && !(eTYPE == SDOGeometryEType.COMPOUND_POLYGON_EXTERIOR)
                && INTERPRETATION != 1
                && INTERPRETATION != 3) {
            return null;
        ...
        for (int i = triplet; (i < endTriplet) && ((etype = ETYPE(elemInfo, i)) != -1); i++) {
            if ((etype == SDOGeometryEType.POLYGON)
                ...
                list.add(poly);
            } else if (etype == SDOGeometryEType.COMPOUND_POLYGON_EXTERIOR
                    || etype == SDOGeometryEType.COMPOUND_POLYGON) {
                Polygon poly = createPolygon(gf, GTYPE, SRID, elemInfo, i, coords);
            ...
            switch (etype) {
                ...
                case SDOGeometryEType.POINT:
                    if (interpretation == 1) {
                        ...
                        throw new IllegalArgumentException(
                                "SDOGeometryEType.POINT requires INTERPRETATION >= 1");
                    }
                ...
                case SDOGeometryEType.POLYGON_INTERIOR:
                    throw new IllegalArgumentException(
                            "SDOGeometryEType 2003 (Polygon Interior) no expected in a GeometryCollection"
                                    + "(2003 is used to represent polygon holes, in a 1003 polygon exterior)");

                case SDOGeometryEType.CUSTOM:
                case SDOGeometryEType.COMPOUND:
                case SDOGeometryEType.COMPOUND_POLYGON:
                case SDOGeometryEType.COMPOUND_POLYGON_EXTERIOR:
                case SDOGeometryEType.COMPOUND_POLYGON_INTERIOR:
